...
impl FirefoxOptions {
    pub fn from_capabilities(capabilities: &mut NewSessionParameters)
                             -> WebDriverResult<FirefoxOptions> {
        if let Some(options) = capabilities.consume("moz:firefoxOptions") {
            let firefox_options = options.as_object()
                .ok_or(WebDriverError::new(ErrorStatus::InvalidArgument,
                                           "'moz:firefoxOptions' capability was not an object"))?;
            let binary = FirefoxOptions::load_binary(&firefox_options)?;
            let profile = FirefoxOptions::load_profile(&firefox_options)?;
            let args = FirefoxOptions::load_args(&firefox_options)?;
            let log = FirefoxOptions::load_log(&firefox_options)?;
            let prefs = FirefoxOptions::load_prefs(&firefox_options)?;
    ...
    fn load_binary(options: &BTreeMap<String, Json>) -> WebDriverResult<Option<PathBuf>> {
        if let Some(path) = options.get("binary") {
            Ok(Some(PathBuf::from(path.as_string()
                .ok_or(WebDriverError::new(ErrorStatus::InvalidArgument,
    ...
    pub fn load_prefs(options: &BTreeMap<String, Json>) -> WebDriverResult<Vec<(String, Pref)>> {
        if let Some(prefs_data) = options.get("prefs") {
            let prefs = prefs_data.as_object()
...
fn unzip_buffer(buf: &[u8], dest_dir: &Path) -> WebDriverResult<()> {
    ...
    for i in 0..zip.len() {
        ...
        let unzip_path = {
            ...
            if is_dir { None } else { Some(dest_path) }
        };
